...
(function () {
  ...
  // ── State ──────────────────────────────────────────────────
  let allProducts = [];
  let cart = JSON.parse(localStorage.getItem('electromart-cart')) || [];
  let isAdminAuthenticated = sessionStorage.getItem('electromart-admin') === 'true';
  ...
  function initPage() {
    ...
    // Update nav-link active states based on URL
    ...
    // Initialize based on which container is in the DOM
  ...
  async function handleAdminLogin() {
    ...
    try {
      ...
      if (doc.exists && doc.data().password === passwordValue) {
        ...
        sessionStorage.setItem('electromart-admin', 'true');
        const modal = document.getElementById('admin-modal');
        if (modal) modal.classList.remove('active');
        loadAdminData();
      } else {
  ...
  async function handleEditSubmit() {
    ...
    try {
      ...
      if (imageInput && imageInput.value) {
        updateData.imageUrl = imageInput.value.trim();
      } else {
        updateData.imageUrl = '';
      }
  ...
  function initAdminLogout() {
    ...
    if (logoutBtn) {
      logoutBtn.addEventListener('click', () => {
        ...
        sessionStorage.removeItem('electromart-admin');
